...
const TrackControls: React.FC<TrackControlsProps> = ({ track }) => {
  const { toggleTrack, updateTrack, removeTrack } = useAudio();
    ...
    <Card className="p-4 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" size="icon" onClick={() => toggleTrack(track.id)}>
      ...
      <div className="flex items-center gap-2">
        ...
        <Slider value={[track.volume]} max={1} step={0.01} onValueChange={handleVolumeChange} className="flex-1" />
      </div>
    </Card>
  );
};
